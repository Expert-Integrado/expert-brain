...
2. TASKS (ciclo de vida, SEM cobrança): garanta que o trabalho desta sessão está refletido no board — save_task do que foi pedido e ainda não virou task (ou update_subtask no card existente, se for derivado dele — derivado nunca vira card novo), update_task (in_progress) do que está em execução, complete_task do que terminou. NÃO liste nem cobre tasks vencidas/pendentes agora — cobrança de atraso acontece só na abertura da sessão.`;
