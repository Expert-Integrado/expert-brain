...
- Compromisso/pendência com dono e prazo NÃO vira nota: vira task (save_task com dedupe_key derivado do id do e-mail/mensagem de origem — retomada e rodada sobreposta não duplicam). EXCEÇÃO: fonte que JÁ É um gerenciador de tarefas (ClickUp, Notion, Trello, Asana) não tem seus itens recriados como task no Brain — extraia só as decisões, padrões e pessoas em volta deles como notas; migrar o board inteiro só se eu pedir explicitamente. Pessoa nova relevante: registre no vault de contatos (que já vem junto na instalação) e mencione.
