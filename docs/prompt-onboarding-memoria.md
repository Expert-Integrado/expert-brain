...
- Dado sensível (saúde, família/relacionamentos, finanças pessoais, salários de pessoas) NUNCA é descartado por ser sensível: segue o portão do modo escolhido (automático: grava direto; com aprovação: entra no bloco separado de sensíveis) e SEMPRE nasce privado — mark_private logo após o save. Privado = invisível pra agentes sem escopo e fora de qualquer tela compartilhada.
...
3. Menu de múltipla escolha das fontes, cada uma com o PORQUÊ em uma linha:
   ...
   - Documentos na nuvem (Google Drive, Notion) — specs, atas e material de referência
...
Crie no meu board (save_task) o projeto da importação:
- 1 task por fonte MARCADA na Fase 0 — inclusive as que ficaram "pendente" na Fase 1 (essas com título "Reconectar: <fonte>" e o motivo da falha no corpo). Fonte conectada: "Importar memória: <fonte>", com o procedimento da Fase 3 no corpo, a janela e o modo escolhido.
...
Pegue a primeira task e execute a colheita — 1 agente menor por fonte, em paralelo quando houver mais de uma aberta. RESPEITE a calibragem da Fase 0: a janela de datas (30d Pro / 90d Max) vale pra toda leitura, e no plano Pro processe no máximo 3 fontes nesta rodada (as demais ficam abertas no board pra próxima).
- Teto de LEITURA por fonte nesta rodada (não só de saída): ~80 e-mails, ~200 mensagens por canal/conversa, ~80 tasks, ~50 reuniões/transcrições, ~100 registros de CRM, ~50 documentos. O que passar do teto fica pra próxima rodada.
...
- Compromisso/pendência com dono e prazo NÃO vira nota: vira task (save_task). Pessoa nova relevante: se o vault de contatos estiver instalado, registre lá e mencione; senão, nota de pessoa normal.
- Cada candidato: título atômico (1 ideia), tldr concreto de até 280 caracteres com data, kind entre os 7 canônicos (concept, decision, insight, fact, pattern, principle, question), domínios, e a FONTE exata.
...
- ChatGPT e Claude.ai: rota recomendada é a CONEXÃO DIRETA — o Brain vira um conector dentro da própria ferramenta e ela mesma despeja TUDO que lembra da pessoa no vault. Me guie assim:
  ...
  * Com o conector ativo, MONTE você os dois textos prontos pro modo escolhido na Fase 0 (automático = save_note; com aprovação = capture) e entregue em blocos de copiar: (1) o prompt de migração completa ("liste TUDO que você lembra sobre mim e salve/deposite cada item — uma ideia por nota, kind adequado, resumo ≤280, origem citada, recall antes pra não duplicar, sensível salva e marca privado"); (2) a instrução de nutrição contínua pras Custom Instructions ("fato durável sobre mim = salvar/depositar no expert-brain; recall antes; sensível nasce privado; consulte o Brain antes de responder sobre meu contexto").
...
Todo candidato passa por: (1) dedupe — recall com 2-3 buscas de ângulos diferentes; já existe = não entra (ou atualiza a existente se o candidato acrescenta fato); (2) fonte sustenta? inferência criativa morre aqui; (3) sensível = segue o portão do modo e nasce privado.
...
- MODO COM APROVAÇÃO: monte o lote agrupado por tema (não uma lista gigante crua), sensíveis SEMPRE num bloco separado no final, e só grave o que eu aprovar (AskUserQuestion por bloco).

FASE 5 — FECHAMENTO E RECORRÊNCIA
- Me mostre o número de nascimento: "seu Brain nasceu com N memórias e M conexões" (stats antes/depois), ABRA o grafo (https://<url-do-brain>/app/graph) pra eu ver o cérebro vivo e o board (https://<url-do-brain>/app/tasks) pra eu acompanhar sozinho as tasks de importação.
...
```text
Liste TUDO que você lembra sobre mim (memórias salvas, preferências, fatos, contexto de projetos) — completo, sem resumir. Pra cada item, deposite um candidato no expert-brain via capture: uma ideia por item, título curto, resumo concreto de até 280 caracteres, citando que a origem é a sua memória interna. Antes de cada capture, rode recall pra não duplicar o que já existe no vault. Ao final, me mostre quantos candidatos entraram na fila — eu reviso tudo pela home do console antes de virar memória.
```
...
```text
Quando você aprender um fato durável sobre mim (decisão, preferência, contexto de projeto, pessoa importante), deposite no expert-brain via capture — uma ideia por item, resumo concreto, citando a origem. Rode recall antes pra não duplicar. Consulte o expert-brain via recall antes de responder perguntas sobre meu contexto.
```
