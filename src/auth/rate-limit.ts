...
// Rate limit de login por IP+e-mail no OAUTH_KV (spec 10-backend/18). Mata
// brute-force barato contra o PBKDF2 capado em 100k iterações (hard limit do
// runtime Workers). KV é eventualmente consistente entre POPs — isto NÃO é um
// contador atômico e não precisa ser: o objetivo é encarecer o ataque burro,
// não garantir contagem exata. Chaves expiram sozinhas (expirationTtl).
//
// O bucket é compartilhado entre POST /authorize e POST /app/login de propósito:
// é a mesma senha, então as falhas somam.
...
export async function registerLoginFailure(env: Env, ip: string, email: string): Promise<number> {
  ...
  try {
    await env.OAUTH_KV.put(key, JSON.stringify(b), {
...
export async function clearLoginFailures(env: Env, ip: string, email: string): Promise<void> {
  try {
    await env.OAUTH_KV.delete(await bucketKey(ip, email));
  } catch {
    // não-fatal
  }
}
