...
// CONVENÇÃO "prazo sem hora" (spec 36 fase 2): o banco só guarda due_at (ms), sem
// flag de "tem hora". Como parseDueToMs de uma data-só ("2026-07-10") gera SEMPRE
// 23:59:00 BRT (fim do dia), adotamos pragmaticamente: 23:59:00 BRT == "só data,
// sem hora". Um prazo intencional de 23:59 é raríssimo e, na prática, indistinguível
// de "até o fim do dia" — o custo de confundir os dois é nulo. Toda exibição usa
// dueHasTime() pra decidir se mostra a hora; o input de HORA nasce vazio nesse caso.
export function dueHasTime(ms: number): boolean {
  const p = brtParts(ms);
  return !(p.h === 23 && p.mi === 59);
}

// "22/06/2026 14:00" em BRT — omite a hora quando o prazo é fim-de-dia-sem-hora.
export function formatBrtDateTime(ms: number): string {
  const p = brtParts(ms);
  const date = `${pad(p.d)}/${pad(p.mo)}/${p.y}`;
  return dueHasTime(ms) ? `${date} ${pad(p.h)}:${pad(p.mi)}` : date;
}

// "22/06 14:00" — versão curta pra badges/cards. Omite a hora quando sem hora.
export function formatBrtShort(ms: number): string {
  const p = brtParts(ms);
  const date = `${pad(p.d)}/${pad(p.mo)}`;
  return dueHasTime(ms) ? `${date} ${pad(p.h)}:${pad(p.mi)}` : date;
}

// "2026-07-10" em BRT — valor pronto pra <input type="date">.
export function brtDateOnly(ms: number): string {
  const p = brtParts(ms);
  return `${p.y}-${pad(p.mo)}-${pad(p.d)}`;
}

// "14:00" em BRT — valor pra <input type="time">. Devolve '' quando é
// fim-de-dia-sem-hora (o input de hora nasce vazio nesse caso).
export function brtTimeOnly(ms: number): string {
  if (!dueHasTime(ms)) return '';
  const p = brtParts(ms);
  return `${pad(p.h)}:${pad(p.mi)}`;
}

// "2026-06-22T14:00" em BRT — valor pronto pra <input type="datetime-local">.
// O input não carrega timezone; o cliente reenvia essa string e o servidor a
// reinterpreta como BRT via parseDueToMs (offset -03:00 fixo). Ver spec 36.
export function brtDatetimeLocal(ms: number): string {
  const p = brtParts(ms);
