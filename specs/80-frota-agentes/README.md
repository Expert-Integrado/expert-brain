...
Adendo 12/07: o grupo ganhou quatro specs além do plano original — **88** (claim/lease, comentários tipados, fila "aguardando o dono"), **89** (watchdog da frota, banner/chip no board, preflight do deploy), **90** (wake fast-path por long-poll — revisão da decisão "latência de minutos é aceitável": segue verdade pra colaboração assíncrona, mas o custo de derrubar pra segundos se mostrou baixo) e **91** (presets de credencial + visibilidade assigned-only: papéis nomeados sobre 3 eixos de escopo no CSV, row-level pra robô em infra compartilhada ver SÓ as tasks dele — zero notas, zero contatos). Visão de usuário no README raiz ("Frota de agentes"); operação por dispositivo em `docs/frota-heartbeat.md`.
