...
Migrations do contacts vivem em `migrations/*.sql` aplicadas via `wrangler d1 migrations apply` (`wrangler.toml` → `migrations_dir = "migrations"`; trilho formalizado na spec `40-ops/44`). Existem hoje `0001` a `0003` — a próxima é `0004`.
...
**1a. Migration aditiva `migrations/0004_similar_edges.sql`** (novo arquivo; NUNCA alterar migrations já aplicadas):
...
- [ ] `migrations/0004_similar_edges.sql` existe, é puramente aditiva e aplica limpa num banco com 0001-0003 já aplicadas (local).
- [ ] `grep -rn "VECTORIZE.query" src/vaults src/web` não mostra NENHUMA chamada dentro de loop por nó no caminho de leitura do grafo (`computeSimilarityEdges` removida; a única query por-load restante é a do `?q=`, que é 1 por request).
- [ ] `save_person`, `save_company`, `save_entity` e `reembed` gravam/atualizam linhas em `similar_edges` (verificável por `SELECT COUNT(*)` antes/depois), e uma falha nessa etapa NÃO falha o save.
- [ ] `POST /setup/backfill-similar` processa em lotes ≤ 20, retorna cursor, é resumível (repetir o mesmo `after` não duplica) e não trava em entidade com vetor órfão.
- [ ] `GET /app/graph/data?vault=contacts` (modo default) retorna arestas `type: 'similar'` lidas do D1, com dedup simétrico e sem pares que já têm edge explícita.
- [ ] Payload do modo `?all=true` permanece sem arestas de similaridade (comportamento atual preservado).
- [ ] `contactsSourceHash` muda após backfill/reembed (cache do grafo auto-invalida).
- [ ] `computeLayout` com 901+ nós retorna sem executar forceAtlas2; com ≤ 900 executa como hoje.
- [ ] Seed de posição usa âncora por cluster (kind) portada da implementação merged da spec `20-frontend/22`, com jitter determinístico (mesmo grafo → mesmo layout).
- [ ] `loadConnectionsBetween` não contém `SELECT` sem `WHERE` sobre `connections`; usa `IN` chunked por `a_id`.
- [ ] Os 3 `SELECT ... LIMIT ${MAX_EDGES}` restantes logam `console.warn` quando o resultado atinge o teto.
- [ ] `tsc --noEmit` limpo e `vitest run` verde com os testes da Parte 4.
...
- `migrations/0004_similar_edges.sql` (novo)
- `test/` (novo) + `package.json` (vitest, scripts `test`/`typecheck`)
