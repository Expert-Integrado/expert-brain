...
| E11 | `20-frontend/27` | contacts | DONE 07/07 (rate-limit KV espelho do Brain em /app/login; SSO single-use por nonce — emissor no brain deployado junto; session-epoch: logout/troca de senha revogam todas as sessoes; 8 testes novos, 321 verdes no contacts; sessoes existentes do Console morrem no deploy — re-logar 1x). Era: rate-limit no login do console standalone |
| E12 | `30-features/33` | brain | DONE 07/07 (reconciliada no trilho de `share.ts`: createShare/resolveShare/revokeShare generalizados pra qualquer nota viva nao-privada; migration 0016 `share_include_media`; pagina publica de NOTA sem comentarios; proxy `/s/<token>/media/<id>` no-store; rate-limit 30/min por IP em /s/*; setNotePrivate revoga share fail-closed; UI no detalhe da nota + aliases /app/notes/share|unshare; client share-ui.ts compartilhado; 24 testes no share.test.ts, 791+5 verdes). Era: share so de task |
| E13 | `30-features/36` | ambos | DONE 07/07 (auditoria: fase 2 JA estava implementada e testada — handleNoteUpdatePost + note-edit.ts + updateNote com expectedUpdatedAt + reembedNoteIfNeeded compartilhado + 13 testes; so checkboxes defasados; zero codigo novo). Era: spec in-progress com fase 2 sem check |
| E14 | `40-ops/45` | contacts | DONE 07/07 — Parte 1 (1.451 categorias no D1) + Parte 2 (canon `categorized`/`seed`, scripts seeds+whatsapp com dry-run default, `docs/categorias-fontes.md`, 6 testes; commit 611a49b, deploy 81feebfa). Apply real da 4ª fonte gateado em OK do dono |
