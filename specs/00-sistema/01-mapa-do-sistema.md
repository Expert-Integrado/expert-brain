...
**b) MCP como Durable Object** — classe `ExpertBrainMCP` (`src/mcp/agent.ts`), binding `MCP_OBJECT` no `wrangler.toml`. As tools são registradas em `src/mcp/registry.ts` (`registerAllTools`): **~22 tools** = 10 de conhecimento (save_note, update_note, delete_note, restore_note, recall, expand, get_note, link, stats, reembed) + 5 de tasks (save_task, list_tasks_due_today, list_tasks, complete_task, update_task) + 3 de mídia R2 (attach_media, get_note_media, delete_note_media) + 4 de contatos read-only (list_contacts, search_contacts, get_contact, get_contact_by_phone — `src/mcp/tools/contacts.ts`, que chamam o Worker de contacts via **service binding `CONTACTS`** declarado no `wrangler.toml`).
