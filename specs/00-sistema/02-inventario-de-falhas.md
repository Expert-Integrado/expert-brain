...
| 15 | Tag de máquina + hook SessionStart | **DESCARTADO** (client-side — ver seção de descartes) | descartado |
...
2. **`backlog-15` (tag de máquina + hook SessionStart listando tasks da máquina):** descartado como spec destes repos porque é 100% client-side — o servidor já suporta o mecanismo inteiro via tags livres (e o filtro de tag case-insensitive entra na spec `10-backend/15`). Hook de SessionStart é configuração pessoal do dono da instância e, pela regra de boundary, não entra no repo open-source; a convenção (tag `machine:<slug>`) fica documentada na configuração pessoal do dono, fora desta árvore.
