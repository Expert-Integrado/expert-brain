...
2. **Sessão do Console.** Cookie `mv_session` assinado por HMAC, emitido com `Path=/app` (`src/web/session.ts:71`), verificado por `requireSession` (`src/web/session.ts:87-102`). Ou seja: o cookie NEM é enviado pelo browser em requisições fora de `/app/*`.
