...
### Resultados medidos (preencher na execução — gate)

| Métrica (N=5k sintético) | Antes | Depois |
| --- | --- | --- |
| Bytes do payload `/app/graph/data` | _medir_ | _medir_ |
| Linhas de `similar_edges` lidas por rebuild | _medir_ | _medir_ |
| Queries D1 por request em cache hit | 3 | 1 |
| Value órfão no KV após bump/rotação | permanente | expira ≤7d |
