...
- **Fusão do vault de contatos num Worker só**: o antigo worker irmão `expert-contacts` (que rodava num deploy separado e conversava com o Brain por service binding) foi **embutido** neste repo — o código vive em `src/contacts/` e roda in-process dentro do Worker único. Não há mais segundo Worker, segundo repo, service binding nem SSO cross-worker: menções `@contato`, a aba **Contatos** do console, os painéis de integração (WhatsApp, Instagram, Pipedrive) e as tools de contato do MCP continuam idênticos, só que servidos pelo mesmo deploy. As rotas públicas de integração ficam nos mesmos paths (`/google/*`, `/whatsapp/*`, `/instagram/*`, `/pipedrive/*`); a API de entidades e o health-check do módulo passam a viver sob o prefixo `/contacts/*` (o health é `/contacts/health`, era `/health` do worker antigo). Os crons do contacts (sync diário, snapshot semanal) viraram braços do dispatcher `*/30` do Brain. Contatos agora vêm **sempre** com o `npm run setup` (sem passo separado): ele cria os recursos próprios do módulo — D1 `expert-contacts-db`, Vectorize `expert-contacts-vec`, `KV_CONTACTS`, R2 `expert-contacts-media` — e gera os tokens internos. Quem não usa, ignora (a aba Contatos nasce com empty state); uma instalação só-Brain anterior à fusão ganha os contatos no próximo `npm run setup`, sem re-perguntar credenciais e sem tocar nos dados existentes.
