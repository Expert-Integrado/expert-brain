"use strict";(()=>{function o(e){return e.replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n])}var x={management:"#f59e0b",sales:"#22c55e",marketing:"#ec4899",education:"#8b5cf6","ai-applied":"#06b6d4",leadership:"#f97316",product:"#3b82f6",operations:"#94a3b8","personal-development":"#14b8a6",entrepreneurship:"#ef4444","cognitive-science":"#a78bfa",music:"#fbbf24"},C={person:"#22c55e",company:"#3b82f6",group:"#a855f7",place:"#f59e0b",event:"#ec4899",other:"#64748b"},_="#64748b";function $(e){return x[e]??C[e]??_}var L={met:"Encontro",talked:"Conversa",meeting:"Reuni\xE3o",email:"E-mail",message:"Mensagem",note:"Nota",saw_post:"Vi post",recommended:"Indica\xE7\xE3o",birthday_reminder:"Anivers\xE1rio",mentioned_in_brain:"Citado no Brain"};var M={person:"Pessoa",company:"Empresa",group:"Grupo",place:"Lugar",event:"Evento",other:"Outro"},N={colleague:"Colega",friend:"Amigo(a)",family:"Fam\xEDlia",client:"Cliente",mentor:"Mentor(a)",alum_g4:"Alumni G4",peer_tech:"Par (tech)",introduced_by:"Apresentado por",works_at:"Trabalha em",founded:"Fundou",advisor_of:"Conselheiro de",studied_at:"Estudou em",member_of:"Membro de",partner_of:"Parceiro de",supplier_of:"Fornecedor de",competitor_of:"Concorrente de",parent_of:"Controladora de",subsidiary_of:"Subsidi\xE1ria de",invested_in:"Investiu em",client_of:"Cliente de",other:"Outro"};function E(e){return N[e]??e}var S=[{value:"met",label:"Encontro"},{value:"talked",label:"Conversa"},{value:"meeting",label:"Reuni\xE3o"},{value:"email",label:"E-mail"},{value:"message",label:"Mensagem"},{value:"note",label:"Nota"}],I=20;function H(e){if(!e)return"";let n=e.includes("T")?e:`${e.replace(" ","T")}Z`,t=new Date(n);if(Number.isNaN(t.getTime()))return e.slice(0,16);try{return t.toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo",day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}catch{return e.slice(0,16)}}function k(e){return e.toISOString().slice(0,19).replace("T"," ")}function A(e){return`/app/contacts/${encodeURIComponent(e)}`}function R(e){e.innerHTML=`
  ...
  `}function O(e,n){let t=n.length?`<div class="contact-page-vinculos">${n.map(i=>`
        ...
        </a>`).join("")}</div>`:'<p class="contact-page-empty">Nenhuma nota menciona esta pessoa ainda.</p>';e.innerHTML=`<h2>Notas sobre esta pessoa</h2>${t}`}function q(e,n,t){let i=n.length?`<div class="contact-page-vinculos">${n.map(l=>`
      ...
      <form class="panel-form" data-timeline-form>
        <div class="panel-form-field">
          ...
          <select class="panel-form-input" data-timeline-kind>
            ${S.map(r=>`<option value="${r.value}">${o(r.label)}</option>`).join("")}
          </select>
        </div>
        <div class="panel-form-field">
  ...
  `;let i=n.querySelector("[data-timeline-list]"),s=n.querySelector("[data-timeline-more]"),l=n.querySelector("[data-timeline-form]"),d=n.querySelector("[data-timeline-kind]"),a=n.querySelector("[data-timeline-context]"),p=n.querySelector("[data-timeline-when]"),m=n.querySelector("[data-timeline-feedback]"),c=n.querySelector("[data-timeline-submit]");function g(r){return`<li>
      <span class="panel-event-kind">${o(L[r.kind]??r.kind)}</span>
      <span class="panel-event-ts">${o(H(r.ts))}</span>
      ${r.context?`<div class="panel-event-ctx">${o(r.context)}</div>`:""}
    </li>`}async function u(){if(!t.loading){t.loading=!0,s.disabled=!0,s.textContent="Carregando...";try{let r=await fetch(`/app/contacts/entity/events?id=${encodeURIComponent(e)}&offset=${t.offset}&limit=${I}`,{credentials:"same-origin"}),f=r.ok?await r.json():null;if(!f||f.ok===!1){s.style.display="none",t.offset===0&&(i.innerHTML='<li class="panel-empty">Erro ao carregar intera\xE7\xF5es.</li>');return}t.total=f.total??0;let b=Array.isArray(f.events)?f.events:[];t.offset===0&&b.length===0?i.innerHTML='<li class="panel-empty">Nenhuma intera\xE7\xE3o registrada ainda.</li>':i.insertAdjacentHTML("beforeend",b.map(g).join("")),t.offset+=b.length,s.style.display=t.offset<t.total?"":"none",s.textContent="Carregar mais"}catch{s.style.display="none"}finally{s.disabled=!1,t.loading=!1}}}if(s.addEventListener("click",()=>{u()}),u(),l.addEventListener("submit",r=>{r.preventDefault(),m.textContent="",m.classList.remove("error","ok");let f=a.value.trim(),b={entity_id:e,kind:d.value};if(f&&(b.context=f),p.value){let v=new Date(p.value);Number.isNaN(v.getTime())||(b.ts=k(v))}c.disabled=!0,c.textContent="Registrando...",fetch("/app/contacts/entity/event",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify(b)}).then(async v=>{let y=await v.json().catch(()=>({}));if(!v.ok||!y.ok)throw new Error(y.error||`falha ${v.status}`);let T=i.querySelector(".panel-empty");T&&T.remove(),i.insertAdjacentHTML("afterbegin",g({kind:d.value,ts:b.ts||k(new Date),context:f||null})),t.total+=1,t.offset+=1,a.value="",p.value="",m.classList.add("ok"),m.textContent="Registrado."}).catch(v=>{m.classList.add("error"),m.textContent=`Erro: ${String(v?.message||v)}`}).finally(()=>{c.disabled=!1,c.textContent="Registrar"})}),location.hash==="#registrar-interacao"){let r=n.querySelector(".panel-addconn");r&&(r.open=!0,r.scrollIntoView({block:"center"}),a.focus())}}async function B(){let e=document.querySelector(".contact-page");if(!e)return;let n=e.dataset.contactId;if(!n)return;let t;try{let a=await fetch(`/app/contacts/entity?id=${encodeURIComponent(n)}`,{credentials:"same-origin"});t=a.ok?await a.json():{ok:!1}}catch{t={ok:!1}}if(!t||t.ok===!1){R(e);return}w(e,n,t);let i=e.querySelector('[data-section="neighbors"]');i&&fetch(`/app/contacts/entity/neighbors?id=${encodeURIComponent(n)}`,{credentials:"same-origin"}).then(a=>a.ok?a.json():null).then(a=>{if(!a||a.ok===!1){i.innerHTML='<h2>V\xEDnculos</h2><p class="contact-page-empty">V\xEDnculos indispon\xEDveis no momento.</p>';return}j(i,a)}).catch(()=>{i.innerHTML='<h2>V\xEDnculos</h2><p class="contact-page-empty">V\xEDnculos indispon\xEDveis no momento.</p>'});let s=e.querySelector('[data-section="mention-notes"]'),l=e.querySelector('[data-section="mention-tasks"]');(s||l)&&fetch(`/app/contacts/entity/mentions?id=${encodeURIComponent(n)}`,{credentials:"same-origin"}).then(a=>a.ok?a.json():null).then(a=>{s&&(!a||a.ok===!1?s.innerHTML='<h2>Notas sobre esta pessoa</h2><p class="contact-page-empty">Indispon\xEDvel no momento.</p>':O(s,a.notes??[])),l&&(!a||a.ok===!1?l.innerHTML='<h2>Tarefas com esta pessoa</h2><p class="contact-page-empty">Indispon\xEDvel no momento.</p>':q(l,a.tasks_open??[],a.tasks_closed_count??0))}).catch(()=>{s&&(s.innerHTML='<h2>Notas sobre esta pessoa</h2><p class="contact-page-empty">Indispon\xEDvel no momento.</p>'),l&&(l.innerHTML='<h2>Tarefas com esta pessoa</h2><p class="contact-page-empty">Indispon\xEDvel no momento.</p>')});let d=e.querySelector('[data-section="timeline"]');d&&D(n,d)}B().catch(e=>console.error("contact-page: fatal",e));})();
