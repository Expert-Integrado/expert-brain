...
> Exceção única: **anexos de mídia nas notas** usam R2, que exige billing habilitado na Cloudflare (mesmo dentro do free tier do R2). O setup detecta contas sem R2 e instala **sem mídia** automaticamente — todo o resto (notas, grafo, tasks, busca) funciona normal, sem cartão.
...
Os ~2.400 tokens vêm das descrições de tool e instruções de uso que o MCP injeta no system prompt ([`src/mcp/instructions.ts`](src/mcp/instructions.ts) + os schemas de tool). *Nota de honestidade:* essa medição é da v1, com 9 tools — hoje o servidor registra ~25 (tasks, share, mídia, contatos), então o overhead real é maior; a ordem de grandeza e as conclusões por plano continuam valendo, mas re-meça no seu setup se o orçamento for apertado. Eles carregam em **toda** requisição enquanto o MCP tá conectado, use você o vault naquele turno ou não. O cache de prompt (TTL de 5 min) torna isso quase grátis em conversa contínua, mas você repaga em todo cold start.
...
npm run setup         # responde 3 perguntas (e-mail + senha + temas opcionais) e espera ~3min
```
...
O `npm run setup` é **idempotente**: se já existe um Expert Brain na sua conta Cloudflare, ele detecta, reaproveita os recursos (D1/Vectorize/KV) e rebuilda + redeploya o Worker, sem mexer em credenciais nem dados. O modo atualização também **instala/atualiza os hooks de captura automática** (seção abaixo) — é assim que instalações antigas ganham o "salvar sozinho". Depois é só dar `Ctrl+Shift+R` no dashboard.
...
### Captura automática (hooks do Claude Code)

O servidor MCP sozinho é **reativo**: ele oferece salvar quando você pede. O que faz o Brain *salvar sozinho* é a **camada cliente** — 6 hooks do Claude Code que o `npm run setup` instala na sua máquina:

| Hook | Quando dispara | O que faz |
|---|---|---|
| SessionStart | Início de cada sessão | Lembra o Claude do comportamento proativo (recall antes de perguntar, salvar ao aprender, tasks do dia) |
| capture-nudge | A cada mensagem sua | Detecta sinais de **prazo** (→ `save_task`), **decisão/insight/métrica/regra** (→ `save_note`) e **contato** (→ expert-contacts, se instalado). Tem cooldown pra não virar spam |
| audit | A cada `save_*` | Registra o save num log local (alimenta a varredura) |
| stop-sweep | Fim de cada turno | Sessão longa sem nenhum save há muito tempo → 1 lembrete de avaliar se há material durável |
| PreCompact | Antes da compactação | Última chance de extrair conhecimento com o contexto inteiro |
| PostCompact | Depois da compactação | Salvar o que sobrou + reancorar as tasks do dia |

O que você precisa saber:

- **Onde mora:** arquivos em `~/.claude/hooks/expert-brain-*.cjs` + entradas no `~/.claude/settings.json`. O instalador faz **backup do settings.json antes de qualquer escrita** e nunca toca nas suas outras configurações.
- **Privacidade:** os hooks não fazem nenhuma chamada de rede. Os logs (`~/.claude/logs/capture-nudge.jsonl`, `brain-saves.jsonl`) são locais, na sua máquina, e servem só pra você medir se a captura está funcionando.
- **Nada é salvo sozinho:** os hooks apenas *lembram* o Claude de considerar salvar — a decisão continua no agente e em você.
- **Temas personalizados:** o setup pergunta (opcional) se você quer palavras/temas seus que sempre merecem a sugestão de salvar. Re-rodar o setup preserva os que você já configurou.
- **Pra desativar:** remova as entradas `expert-brain-*` do `~/.claude/settings.json` (ou rode `/hooks` no Claude Code) e apague os arquivos em `~/.claude/hooks/`. O Brain volta ao modo reativo.
- **Noutra máquina** (sem re-rodar o setup): `node scripts/install-claude-hooks.mjs https://<seu-worker>.workers.dev`.
...
- `MEDIA` *(opcional)* — bucket R2 pra anexos de nota; sem ele o Brain sobe sem mídia
- `CONTACTS` + `CONTACTS_PROXY_TOKEN` *(opcionais)* — service binding pra um Worker de contatos separado; sem eles as tools de contatos não são registradas
...
Além do núcleo de conhecimento acima, o servidor registra os grupos:

| Grupo | Tools | Função |
|---|---|---|
| **Tasks** | `save_task`, `get_task`, `list_tasks`, `list_tasks_due_today`, `update_task`, `complete_task` | To-dos com status/prazo/prioridade (kind='task', fora do grafo e do recall). Board em `/app/tasks`. |
| **Share** | `share_task`, `unshare_task` | Link público read-only `/s/<token>` de UMA task, com expiração obrigatória e revogação imediata. |
| **Mídia** *(opcional)* | `attach_media_to_note`, `get_note_media`, `delete_note_media` | Anexos em notas via R2 com dedup SHA-256. Só registradas se o binding `MEDIA` existir (R2 requer billing). |
| **Contatos** *(opcional)* | `list_contacts`, `search_contacts`, `get_contact`, `get_contact_by_phone` | Leitura de um vault de contatos separado via service binding `CONTACTS` — infra à parte, não faz parte deste repo. Sem o binding, nem as tools nem as instructions aparecem. |
