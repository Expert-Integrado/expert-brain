...
const TASKS = [
  ...
  'update_task', 'complete_task', 'comment_task', 'claim_task',
];
...
const ALL_36 = [...NOTES, ...NOTES_MEDIA, ...TASKS, ...TASKS_SHARE, ...CONTACTS, ...MAILBOX, ...USERS].sort();
...
const FLEET_13 = [...TASKS, ...TASKS_SHARE, ...MAILBOX, ...USERS].sort();
// Robô colaborador: idem MENOS share (link público /s/ = exfiltração sob assigned-only).
const TASK_WORKER_11 = [...TASKS, ...MAILBOX, ...USERS].sort();

describe('registerAllTools — snapshot literal por preset (spec 91)', () => {
  it('sanidade: as famílias somam exatamente 36 tools sem duplicata', () => {
    expect(ALL_36).toHaveLength(36);
    expect(new Set(ALL_36).size).toBe(36);
  });

  it("preset 'personal-full' (full,private) registra as 36", () => {
    expect(toolsFor(presetById('personal-full')!.scopes).names).toEqual(ALL_36);
  });

  it("preset 'personal' (full) registra as 36", () => {
    expect(toolsFor(presetById('personal')!.scopes).names).toEqual(ALL_36);
  });

  it('sessão OAuth (scopes ausente) = personal-full: 36', () => {
    expect(toolsFor(undefined).names).toEqual(ALL_36);
  });
  ...
  it("preset 'fleet-worker' registra EXATAMENTE as 13 (tasks + share + mailbox + users)", () => {
    expect(toolsFor(presetById('fleet-worker')!.scopes).names).toEqual(FLEET_13);
  });

  it("preset 'task-worker' registra EXATAMENTE as 11 (fleet menos share)", () => {
    expect(toolsFor(presetById('task-worker')!.scopes).names).toEqual(TASK_WORKER_11);
  });
